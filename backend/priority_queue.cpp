...
const string LOCATIONS[100] = {"Connaught Place",
                               "Lajpat Nagar",
                               "Karol Bagh",
                               "Saket",
                               "Dwarka Sector 10",
...
double calculateTimeToEmpty(const ATM &atm) {
  if (atm.cashLevel <= 0.0)
    return 0.0;
  if (atm.dailyWithdrawalRate <= 0.0)
...
string getStatus(double timeToEmpty) {
  if (timeToEmpty <= 2.0)
    return "CRITICAL";
  else if (timeToEmpty <= 6.0)
    return "HIGH";
  else if (timeToEmpty <= 12.0)
    return "MEDIUM";
  else
    return "LOW";
}
...
void saveToFile(const vector<ATM> &atms, const string &filename) {
  ...
  for (const ATM &a : atms) {
    ...
    out << a.id << "," << a.name << "," << a.location << "," << a.x << ","
        << a.y << "," << a.cashLevel << "," << a.dailyWithdrawalRate << ","
        << a.daysSinceRefill << ","
...
int main() {
  ...
  cout << left << setw(5) << "Rank" << setw(9) << "ATM ID" << setw(26)
       << "Location" << setw(13) << "Cash Level%" << setw(13) << "Daily Rate"
       << setw(19) << "Days Since Refill" << setw(16) << "Time Left (hrs)"
       << setw(10) << "Status"
       << "\n";
  cout << string(111, '-') << "\n";
  ...
  for (const ATM &a : sorted) {
    ...
    if (s.find("CRITICAL") != string::npos)
      criticalCount++;
    else if (s.find("HIGH") != string::npos)
      highCount++;
  }
  ...
  for (int rank = 1; rank <= 20; rank++) {
    ...
    cout << left << setw(5) << rank << setw(9) << a.name << setw(26)
         << a.location << setw(13) << fixed << setprecision(1) << a.cashLevel
         << setw(13) << fixed << setprecision(0) << a.dailyWithdrawalRate
         << setw(19) << a.daysSinceRefill << setw(16) << fixed
         << setprecision(2) << display_tte << setw(10) << status << "\n";
  }

  vector<int> route;
  for (auto &atm : atms)
    route.push_back(atm.id);
